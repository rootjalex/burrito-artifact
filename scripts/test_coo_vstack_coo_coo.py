...
def f_burrito(m0, m1):
    return burrito.coo_vstack_coo_coo(m0, m1)

def process_scipy(matrix):
    return split_matrix(matrix)

def f_scipy(m0, m1):
    return scipy.sparse.vstack([m0, m1])
...
def f_pydata(m0, m1):
    return sparse.concatenate([m0, m1], axis=0)
...
if __name__ == "__main__":
    tests = [
        tester.Tester("coo_vstack_coo_coo", "coo", process_burr, f_burrito, process_scipy, f_scipy, process_pydata, f_pydata, f_equals),
    ]
