...
def f_burrito(matrix):
    return burrito.csr_slice_1d_csr(matrix, sN, eN, rN)

def process_scipy(matrix):
    return [matrix]

def f_scipy(matrix):
    return matrix[sN:eN:rN, :]

def process_pydata(matrix):
    return [sparse.GCXS.from_scipy_sparse(matrix)]

def f_pydata(matrix):
    return matrix[sN:eN:rN, :].asformat("csr")
...
if __name__ == "__main__":
    tests = [
        tester.Tester("csr_slice_1d_csr", "csr", process_burr, f_burrito, process_scipy, f_scipy, process_pydata, f_pydata, f_equals),
    ]
