...
def f_burrito(m0, m1):
    return burrito.coo_hstack_csr_csr(m0, m1)

def process_scipy(matrix):
    return split_matrix(matrix)

def f_scipy(m0, m1):
    return scipy.sparse.hstack([m0, m1], format="coo")
...
def f_pydata(m0, m1):
    return sparse.concatenate([m0, m1], axis=1).asformat("coo")
...
if __name__ == "__main__":
    tests = [
        tester.Tester("coo_hstack_csr_csr", "csr", process_burr, f_burrito, process_scipy, f_scipy, process_pydata, f_pydata, f_equals),
    ]
