...
def f_burrito(m0, m1):
    return burrito.csr_hstack_csr_csr(m0, m1)
...
def f_pydata(m0, m1):
    return sparse.concatenate([m0, m1], axis=1).asformat("csr")

def f_equals(b, s, p):
    # print(b, s, p)
    b = utils.from_burrito_array(b)
    p = p.to_scipy_sparse().tocsr() # TODO: what???
    # print(f"Formats: {b.format}, {p.format}, {s.format}")
    # print(b.indptr, s.indptr)
    # print(b.indices, s.indptr)
...
if __name__ == "__main__":
    tests = [
        tester.Tester("csr_hstack_csr_csr", "csr", process_burr, f_burrito, process_scipy, f_scipy, process_pydata, f_pydata, f_equals),
    ]

    tester.run_partition(10, 0, 1, tests)
    # tester.run(10, "csr", process_burr, f_burrito, process_scipy, f_scipy, process_pydata, f_pydata, f_equals)
