...
def f_burrito(m0, m1):
    return burrito.csr_vstack_csr_csr(m0, m1)

def process_scipy(matrix):
    return split_matrix(matrix)

def f_scipy(m0, m1):
    return scipy.sparse.vstack([m0, m1], format="csr")
...
def f_pydata(m0, m1):
    return sparse.concatenate([m0, m1], axis=0).asformat("csr")
...
if __name__ == "__main__":
    tests = [
        tester.Tester("csr_vstack_csr_csr", "csr", process_burr, f_burrito, process_scipy, f_scipy, process_pydata, f_pydata, f_equals),
    ]
